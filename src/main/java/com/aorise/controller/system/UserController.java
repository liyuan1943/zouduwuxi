...
public class UserController {
    ...
    public String updateUserPwd(@ApiParam(value = "用户id", required = true) @RequestParam(value = "userId", required = true) String userId,
        ...
        SysUserModel sysUserModel = sysUserMapper.selectById(Integer.parseInt(userId));
        ...
        if (!Utils.getMd5DigestAsHex(oldPassWord).equals(sysUserModel.getPassWord())) {
        ...
        sysUserModel.setPassWord(Utils.getMd5DigestAsHex(newPassWord));

        //调用接口
        int ret = sysUserMapper.updateById(sysUserModel);
        //返回参数判断
        if (ret > 0) {
    ...
    public String findObjectByuserid(@ApiParam(value = "用户id", required = true) @PathVariable(value = "userId", required = true) String userId) {
        ...
        //调用接口
        SysUserModel model = sysUserMapper.selectById(Integer.parseInt(userId));
        return new JsonResponseData(true, StatusDefineMessage.getMessage(StatusDefine.SUCCESS), StatusDefine.SUCCESS,
                "", model).toString();
